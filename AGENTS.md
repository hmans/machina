...
- `src/ui_layout.zig` owns retained UI layout resolution shared by rendering and scene UI input.
...
- Entity data is authored as component tables such as `[entities.components."machina.transform"]`, `[entities.components."machina.geometry.primitive"]`, `[entities.components."machina.material.surface"]`, `[entities.components."machina.camera"]`, `[entities.components."machina.light.directional"]`, shadow markers like `[entities.components."machina.shadow.caster"]`, UI tables like `[entities.components."machina.ui.rect"]`, `[entities.components."machina.ui.text"]`, `[entities.components."machina.ui.command"]`, `[entities.components."machina.ui.scroll_view"]`, `[entities.components."machina.ui.vbox"]`, `[entities.components."machina.ui.hgroup"]`, and `[entities.components."machina.ui.layout.item"]`, and project-local tables like `[entities.components.spin]`.
...
- First-slice UI uses retained scene components: `machina.ui.canvas`, `machina.ui.rect`, `machina.ui.border`, `machina.ui.text`, `machina.ui.button`, `machina.ui.hit_area`, `machina.ui.command`, `machina.ui.scroll_view`, `machina.ui.vbox`, `machina.ui.hgroup`, `machina.ui.stack`, `machina.ui.layout.item`, `machina.ui.spacer`, `machina.ui.text_block`, `machina.ui.toggle`, `machina.ui.progress_bar`, and `machina.ui.separator`.
...
- `machina.ui.hgroup` lays out direct children horizontally from `position` inside explicit `size`, `spacing`, and `padding`; it redistributes extra width to children with positive `machina.ui.layout.item.grow`.
...
- `machina.ui.layout.item` attaches an entity to a parent by stable entity id and `order`; do not use dense runtime entity indices as UI parent references. It also carries `min_size`, `grow`, and `align`; `grow` redistributes extra main-axis space in `machina.ui.hgroup` and is currently only metadata for older stack/vbox paths.
...
- `machina.ui.hit_area` is a non-rendering interaction rectangle for controls whose hit target differs from their visual primitive. Pair it with `machina.ui.button` and `machina.ui.command` instead of adding private pointer rectangle checks.
...
- Scene-authored UI input routing must use the retained layout model (`scroll_view`, `vbox`, `hgroup`, `stack`, and `layout.item`) for hit testing and scroll handling. Do not hit-test raw `machina.ui.rect.position` unless you have first resolved parent layout and clipping.
...
- Retained command buttons should route through `ui_layout.commandAt` after any needed viewport/design-space conversion. Do not add one-off editor or example button rectangle checks when generated `machina.ui.rect` or `machina.ui.hit_area` plus `machina.ui.button` + `machina.ui.command` data can represent the control.
...
- Editor splitters render as thin `machina.ui.separator` dividers, and their wider hover/click target is a public `machina.ui.hit_area` command button. Keep splitter hit testing routed through `ui_layout.commandAt`.
- Headful editor cursor shape should reflect hovered/active editor controls when practical; splitter hover/drag uses the platform east-west resize cursor.
...
- Editor sidebars should use consistent panel padding. The system inspector is one retained table inside one panel, with aligned text rows but no per-row panel rectangles or row dividers. Component inspector boxes remain full-width stacked boxes with one-pixel separators because those are separate components.
...
- Prefer reusable retained UI layout primitives over one-off renderer/editor layout shortcuts. Use `machina.ui.scroll_view` for clipped scrollable regions, `machina.ui.vbox` for vertical stacks, `machina.ui.hgroup` for resizable horizontal editor/tool regions, and `machina.ui.layout.item` with stable entity-id parents for child ordering.
...
- Route scroll-wheel handling through `ui_layout.routeScrollWheelAt`/`applyScrollWheelAt` against retained `machina.ui.scroll_view` data. Do not add editor-only or example-only scroll hit tests when a generated UI component can represent the target.
- Route button command handling through `ui_layout.commandAt` against retained `machina.ui.rect`, `machina.ui.button`, and `machina.ui.command` data. Editor chrome may consume editor commands directly, but it should still use generated retained UI components for hit ownership.
