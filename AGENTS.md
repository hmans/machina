...
Luau is the target scripting language. The current implementation parses a constrained Luau declaration surface for `ecs.component(...)`, `ecs.schema(...)`, `ecs.fields(...)`, `ecs.query(...)`, and `ecs.system(...)`, stores real Luau system callbacks, and runs them through the native scheduler. The preferred script API uses typed component handles and query objects: `ecs.component(...)` can infer Vec3-backed payload types from `ecs.schema({ field = ecs.vec3() })`, `ecs.component<<T>>(...)` remains available for explicit schemas, `ecs.fields(...)` preserves typed string field maps for explicit declarations, `ecs.query(...)` creates a reusable typed query object, systems attach that query object with `query = ...`, and runtime loops use `Query:iter(world)` to yield the entity plus requested component proxies. Use `ecs.refs(...)` for explicit write access or extra manual access declarations. Keep the public API system-first: scripts declare component access, and the native engine owns scheduling, validation, reload transactions, and future parallelization.
