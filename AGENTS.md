...
Luau is the target scripting language. The current implementation parses a constrained Luau declaration surface for `ecs.component(...)` and `ecs.system(...)`, stores real Luau system callbacks, and runs them through the native scheduler. The preferred script API uses typed component handles: `ecs.component<<T>>(...)` returns a handle, `reads`/`writes` can use those handles, and `world.query(...)` yields the entity plus requested component proxies. The example project declares a project-local `spin` component and rotates cubes by querying typed `machina.transform` and `spin` handles. Keep the public API system-first: scripts declare component access, and the native engine owns scheduling, validation, reload transactions, and future parallelization.
