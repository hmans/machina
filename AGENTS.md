...
Rendering uses `wgpu-native`. Headful rendering currently uses SDL3 on macOS via Homebrew paths in `build.zig`; offscreen rendering writes BMP artifacts and is the preferred automation surface. Renderable meshes, the active camera fallback, and the first directional light are resolved from ECS world data where available. New scene-authored renderables should use `machina.geometry.primitive` plus `machina.material.surface`; `machina.render.cube` is a legacy shortcut that renders as box geometry with inline color. The renderer owns an internal render world and render-phase schedule built with the same `runtime.World`, component registry, and scheduler implementation as game worlds. Matching geometry/material renderables are automatically grouped into instanced render batches below the scene authoring surface. GPU handles remain renderer-owned side resources until native/internal component storage has explicit lifecycle rules.
