...
- Scenes with at least 256 occupied instance slots build a max-depth Hi-Z pyramid after the depth prepass only on stable-instance frames. The following frame consumes it only while the camera matrix and persistent instance records remain unchanged. Camera movement or transform, membership, geometry, material, or LOD changes temporarily fall back to frustum culling rather than building or consuming depth that cannot be reused safely.
