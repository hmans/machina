...
- WGPU geometry and materials are cached by resource handle and version. Project and editor UI domains use monotonic layout and paint revisions raised by typed ECS mutation, scrolling, and interaction, so unchanged frames avoid a whole-tree component hash and retain their paint commands. The retained UI stream is independently content-signatured at the backend boundary, while dynamic camera and gizmo primitives use a separate small overlay buffer; moving editor-world tooling therefore does not rebuild or upload the complete chrome vertex stream.
...
- Inspect structured `render_stats` for GPU-driven mode, draw/slot/visibility capacity, database rebuilds, occupied slot span, cumulative instance upload calls/bytes, frustum candidates, occlusion rejects, per-LOD visible counts, and UI vertex rebuild/upload counts and bytes.
