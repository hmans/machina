...
Open the editor to compare individual engine, project-Odin, and Luau systems. The Systems panel publishes a rolling 50-frame average every five frames. Engine phases and project systems measure their CPU callback boundaries. Renderer work is split across `scrapbot.render.cull`, `.shadow`, `.world`, `.post`, `.ui`, `.finish`, `.submit`, and `.present`; these are CPU encoding and API timings. Structured WGPU `render_stats` separately expose asynchronous GPU execution samples.

The collapsible Performance panel gives a compact whole-frame view beside the Systems panel. It publishes every five frames and shows FPS and CPU frame duration from a rolling 50-frame window, the latest valid asynchronous GPU frame duration, scene-plus-runtime entity count, retained GPU draw-batch count, and the number of instances rejected by Hi-Z occlusion. GPU frame time remains `--` until timestamp-query results are available. A draw batch is Scrapbot's GPU-driven geometry/material grouping unit, not a count of every graphics API draw command across every pass.
...
- Renderable membership follows structural and value dirty queues. The retained render list updates only changed entities and slots; an unchanged frame does not scan the active renderable set. Cameras and the bounded light sets remain cheap frame values because camera motion and lighting directly affect the render uniform.
- WGPU addresses persistent instance records by stable ECS render slot, coalesces nearby changed slots into bounded uploads, and retains compact render/culling uniforms until their values change. Instance-to-LOD batch mappings and indirect templates are likewise retained instead of being reconstructed on stable frames. Spawn and despawn within an existing geometry/material/LOD batch adjust retained membership without rebuilding the draw database; new batch keys or required capacity can still grow it. The backend computes camera and shadow visibility on the GPU and obtains per-batch instance counts through indexed indirect arguments. It retains a 131,072-slot limit; its draw database grows geometrically instead of imposing a fixed batch count.
- Scenes with at least 256 occupied instance slots build a max-depth Hi-Z pyramid after the depth prepass. The following frame consumes it only while the camera matrix and persistent instance records are unchanged. Camera movement or transform, membership, geometry, material, or LOD changes temporarily fall back to frustum culling rather than risking rejection from stale depth.
...
- Editor entity and resource browsers refresh at tool cadence. The inspector instead follows the selected entity's component revision or selected resource version, so unrelated runtime churn does not rebuild stable component panels. Profiler revisions update only profiler rows and direct manipulation stays frame-responsive.
...
- Use the Performance panel for whole-frame health, the Systems panel for CPU phase attribution, and `tests/fixtures/ui/ui-performance.json` for editor interaction costs.
- Use bounded headless WGPU plus a framegrab when renderer correctness or submission cost matters.
- Inspect structured `render_stats` for GPU-driven mode, draw/slot/visibility capacity, database rebuilds, occupied slot span, cumulative instance upload calls/bytes, frustum candidates, occlusion rejects, per-LOD visible counts, and UI vertex rebuild/upload counts and bytes. `ui_project_vertex_rebuilds`, `ui_editor_vertex_rebuilds`, and `ui_overlay_vertex_rebuilds` identify which retained domain invalidated.
