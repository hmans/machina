...
Pause preserves the current runtime world so Play can resume it. Stop intentionally returns to the source-authored scene baseline without reloading code. Stopped is authoring mode: inspector, gizmo, entity, and component changes mark the scene dirty, and the bottom bar reports `STOPPED / UNSAVED`. Save—or `Ctrl/Cmd+S`—writes those changes explicitly. Play and Step remain blocked until the changes are saved or discarded with Stop.
