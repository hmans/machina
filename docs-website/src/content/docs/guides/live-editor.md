...
Texture and Model resources expose import information in the same inspector: source dependency, product kind and size, warnings, errors, and status. Texture inspectors include a bounded sampled thumbnail. Model inspectors use the public `scrapbot.ui_viewport` component to render the imported geometry and materials with real depth, perspective, and lighting; drag the preview to orbit and use the wheel to zoom. Stable previews are revision-cached. Click **Reimport** to force only the selected Texture or Model importer, update its live registry entry, and reconcile model instances without restarting Luau, native Odin, or the scene world. **Reimport All** in the Resources toolbar forces every imported declaration. Failed imports leave the prior atomic product intact and surface the error in the inspector.
