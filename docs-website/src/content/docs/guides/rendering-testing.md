...
WGPU retains geometry/material/LOD batches across transform-only frames. Stable ECS render slots address a backend-owned instance table, and only changed contiguous ranges are uploaded. One compute pass produces separate camera-visible and shadow-visible instance lists plus their indirect counts. The renderer supports 131,072 instance slots and grows its draw database geometrically; it uses conservative geometry-derived bounding spheres and one portable indirect call per retained batch.
