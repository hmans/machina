...
24. Port `wgpu-native` bindings and offscreen render verification. The first Odin slices should establish ABI-compatible scalar types, string/chained structs, renderer texture formats, usage flags, texture/buffer descriptors, instance/adapter/device/queue request descriptors and procs, bind-group/sampler/pipeline-layout descriptors and procs, render pass descriptors and draw command procs, offscreen copy/readback procedure types, typed proc-table resolution, and dynamic library loading before wiring real WebGPU render execution.
