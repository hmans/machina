...
Machina needs a native, cross-platform implementation language suitable for engine runtime code, tooling, tests, and a single distributable binary. C and C++ are explicitly undesirable. Rust offers strong ecosystem advantages, especially around graphics, serialization, tooling, and package management, but it also introduces complexity around ownership, lifetimes, unsafe boundaries, compile times, and agent-generated refactors.
...
Machina gives up some of Rust's ecosystem advantages. The project will need to own more glue code for serialization, scripting bindings, build packaging, file watching, and native dependencies.
