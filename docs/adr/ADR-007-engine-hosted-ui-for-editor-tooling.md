...
Machina is expected to grow an editor, but the editor should not be a separate application stack disconnected from the runtime. Engines such as Godot demonstrate the value of building editor tooling with the engine's own UI and rendering systems.
...
Machina will provide UI primitives inside the engine and use them to build editor tooling.
...
Building an engine-hosted editor is slower initially than using an external GUI framework, but it keeps Machina coherent and makes editor behavior more portable across interactive and headless-adjacent workflows.
