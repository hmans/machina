...
17. Port first-pass test command discovery, manifest validation, field assertion execution, script-visible input resource replay, and retained scene UI command/scroll routing while editor-shell input routing and native-backed fixtures still wait for their Odin ports.
