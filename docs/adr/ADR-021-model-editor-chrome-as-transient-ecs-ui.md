...
The runtime world contains additional transient editor entities, so systems that report or expose project content must filter by origin when appropriate. Internal role components and the renderer's project/editor domains remain engine implementation details. Persisted workspace layout, editable inspector controls, and editor command history remain future work.
