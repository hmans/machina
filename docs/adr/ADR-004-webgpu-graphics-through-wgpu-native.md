...
**Odin migration note:** The first Odin migration slices own the render-relevant `wgpu-native` C ABI scalar types, string/chained structs, texture formats, buffer/texture usage flags, initial texture/buffer descriptor structs, instance/adapter/device/queue request descriptors, bind-group/sampler/pipeline-layout descriptors, shader module and render pipeline descriptors, render pass attachment/descriptors, offscreen copy/readback, queue upload, surface creation/configuration/current-texture/presentation, and render pass command procedure boundaries, offscreen proc-table resolver, dynamic-library-backed offscreen proc loader, default `zig-pkg`/environment library discovery, `wgpu-check` diagnostics, instance creation/release smoke, adapter/device/queue context smoke, native `wgpuDevicePoll`, a real offscreen clear/readback smoke, a real WGSL pipeline draw/readback smoke, a tiny WebGPU PNG/BMP artifact command, first-pass scene-driven `render --backend wgpu` image output, bounded hidden `run --backend wgpu` final-frame output, SDL native surface descriptor extraction, hidden SDL WebGPU surface presentation smoke coverage, bounded hidden `run --backend wgpu` scene surface presentation, and bounded/unbounded visible software SDL event loops. They can load a compatible dynamic library, acquire/release a first WebGPU context, submit tiny offscreen clear and triangle draw passes, map a staging buffer, validate readback pixels, write a 1x1 offscreen triangle image through `scrapbot wgpu-render-test`, render scene-derived placeholder rectangles through WebGPU for `render`/`render-test`/bounded hidden `run`, create an SDL window with a platform-native surface descriptor, configure a compatible WebGPU surface, acquire the current surface texture, submit a clear or scene-derived render pass, present hidden frames through `scrapbot wgpu-surface-check` and bounded hidden `scrapbot run --backend wgpu`, and drive visible software runs through SDL event polling, but do not yet support visible WebGPU presentation, visible pixel presentation, or replace the full Zig WebGPU renderer.
