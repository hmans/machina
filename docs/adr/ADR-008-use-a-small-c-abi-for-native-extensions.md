...
Load project native extensions from `build/extensions` as platform dynamic libraries and look for a `scrapbot_extension_register` export. The first ABI is a small C-compatible table in `extension_api`: an ABI version, opaque host userdata, and a function for registering library component schemas.
...
The C ABI keeps the boundary language-neutral and avoids coupling extensions to Scrapbot internals. The tradeoff is that the API must use plain data shapes and explicit versioning. Extension libraries must remain loaded while any runtime registry copied from their cstring metadata is in use.
