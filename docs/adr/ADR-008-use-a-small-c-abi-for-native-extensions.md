...
Let projects declare native extension targets in `project.toml`, build those targets into `build/extensions`, load the resulting platform dynamic libraries, and look for a `scrapbot_extension_register` export. The ABI is a small C-compatible table in `extension_api`: an ABI version, opaque host userdata, functions for registering library component schemas and systems, and a callback context for scheduled system execution and deferred lifecycle commands.
...
The native extension surface remains intentionally narrow: component schema registration, system registration, query-by-component-name, transform access, mesh payloads for cube renderables, vec3 field access for schema-backed custom components, and deferred lifecycle commands. That is enough to prove dynamic loading, registry integration, generated types, hot reload, native system execution, and native structural ECS mutation without exposing internal ECS storage, allocators, or threading primitives.
