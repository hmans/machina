...
Machina stores entities densely. Removing an entity can move the last entity into the removed index so entity iteration and component table storage stay compact. With index-only handles, a stale handle could accidentally refer to the moved entity after removal.
...
The C ABI is wider because entity generation travels with entity index. Hot paths pay a small validation cost, but the safety property is worth it until Machina has a fuller entity id/slot lifecycle model.
