...
The transaction value set covers numeric and boolean property changes, including three-axis transform gestures, plus structural snapshots for create, duplicate, rename, delete, promote, and component membership changes. Revert deliberately reloads disk-authored entities and clears the bounded history instead of manufacturing an inverse transaction for every discarded edit. Multi-selection will require larger transaction payloads, but it can extend the same history, dirty-candidate, and baseline-comparison flow without changing the persistence boundary.
