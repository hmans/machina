...
Render world geometry into a linear floating-point HDR target. Extract bright pixels into a multi-resolution bloom chain, blur each scale, composite the scales with the HDR scene, and tone map once into the presentation target. Render project UI, gizmos, and editor chrome afterward as an ordinary display-referred overlay.
...
The backend now owns several size-dependent floating-point targets and extra fullscreen passes. Window resize must rebuild them, and future exposure and bloom controls need an explicit project-facing settings surface.
