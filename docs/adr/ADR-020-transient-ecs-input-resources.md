...
Machina represents current-frame host input as transient engine-owned ECS components on the shared `machina.input.frame` entity:

- `machina.input.pointer` stores pointer position, pointer delta, primary and secondary button edge/held state, and wheel delta.
- `machina.input.keyboard` stores modifier state, semantic movement key state, and editor-toggle edge state.
- `machina.input.frame` stores frame-level UI visibility and viewport data.
...
- Controller/gamepad support can add more `machina.input.*` components without changing the basic model.
