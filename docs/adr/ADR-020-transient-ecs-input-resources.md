...
SDL and other future platform backends translate raw events into the host-side frame snapshot. The live project writes a routed version of that snapshot into the game world before UI command routing and update systems run. During the current renderer transition, frame-local UI/editor overlay systems also receive the same shape in their transient render-state storage.
