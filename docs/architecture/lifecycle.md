...
| Project/world hot reload | Project, scene, asset, resource, extension binary/source stamp changes | Build and validate a next world from disk, then replace active world | Re-register fonts/materials/LOD geometry in stable slots; increment versions/generations/topology as required | Rebuild native extensions and script runtime, then atomically replace runtime/extension/schedule ownership | New world bootstraps structural/render/UI state; backend caches follow resource revisions | Failed replacement destroys candidate world/runtime and retains/restores the last-good runtime; resource registrations happen before candidate validation and are not currently transactionally rolled back |
