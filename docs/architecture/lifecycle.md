...
| Editor resource reimport | Explicit Reimport for one Texture/Model UUID or Reimport All | Preserve the active world; reconcile Model-derived children after registry mutation | Force selected importer(s), update live entries, retire removed generated Geometry/Material outputs | Preserve Luau runtime, native extensions, and schedule | Exact resource versions/generations invalidate backend consumers; inspector refreshes import state and preview | Atomic product write preserves the last-good artifact; the editor retains and displays the import error |
