...
| `scrapbot.render.post` | Encodes HDR bloom and compositing into the presentation target. | WGPU frames; individual effects remain conditional. | `render/wgpu_post.odin` |
...
- **Outputs:** Visible instance compaction, indirect draw arguments, visibility/LOD counters and next-pass state.
