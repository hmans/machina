...
| Geometry/material descriptions and handles | `resources.Registry` | Runtime shared-resource authority | Project resource load/edit/hot reload; generational handles and content/topology versions expose changes. A monotonic material revision makes content edits invalidate dependent retained GPU records without stable-frame scans. |
...
| Embedded UI viewport membership and layers | `ui.State` / WGPU backend | Derived from authored `scrapbot.ui_viewport` plus layout | Structural UI dirtiness maintains compact viewport-node membership. Layout refreshes only the bounded visible surfaces. WGPU owns fixed texture/depth layers; static Model targets cache by component/aspect and model/geometry/material revisions, while World targets consume the retained render list. |
