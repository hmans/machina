...
- Texture, Model, and Environment declarations retain UUID-backed registry handles and entry versions. Image-based-lighting selection/settings and optional background presentation live on the scene's singleton `scrapbot.world_environment` component; the fixed environment phase resolves its UUIDs into a retained registry cache. Changing either resolved handle, its version, or presentation settings bumps one environment revision consumed by WGPU. A visible empty background reuses the lighting Environment, or selects the procedural atmosphere when both UUIDs are empty. The active camera contributes a separate exposure multiplier, and the first directional light supplies the procedural sun. WGPU samples an imported background's source-resolution panorama at zero blur and its prefiltered cube for intentional blur, while lighting uses the active environment's compact cubes. Imported model registration publishes ordinary Geometry and Material handles for every primitive.
