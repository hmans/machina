...
- Texture, Model, and Environment declarations retain UUID-backed registry handles and entry versions. Image-based-lighting selection/settings and the optional background selection/settings live independently in project render configuration; changing either resolved handle, its version, or presentation settings bumps one environment revision consumed by WGPU. Backgrounds are hidden by default and fall back to the lighting Environment only when explicitly made visible without another UUID. The active camera contributes a separate exposure multiplier. WGPU samples the selected background's source-resolution panorama at zero blur and its prefiltered cube for intentional blur, while lighting uses the active environment's compact cubes. Imported model registration publishes ordinary Geometry and Material handles for every primitive.
