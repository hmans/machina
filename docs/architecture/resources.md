...
- Texture inspection derives a bounded sampled thumbnail from registry pixels. Model inspection targets the public `scrapbot.ui_viewport` component at the Model UUID, so the backend renders real generated Geometry and Materials with depth, lighting, orbit, and zoom. Stable Model layers are cached by component/aspect and resource revisions. Import state, dependency path, product type/size, and the last explicit failure remain editor presentation over registry/import state rather than new resource authority.
