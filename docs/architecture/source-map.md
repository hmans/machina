...
| `src/scrapbot/resources/` | Runtime geometry/material registry and generational handles. | Shared descriptions outside ECS; backend caches consume versions. |
...
| `src/scrapbot/render/` | Backend interface, null backend, WGPU rendering, GPU-driven visibility, picking, gizmos, postprocess. | Backend-neutral inputs; WGPU owns GPU state and caches. |
