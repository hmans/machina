...
- [x] Odin `test` can discover test projects, validate test manifests, replay script-visible input resources, route retained scene UI commands/scroll, consume editor-chrome pointer input, replay editor play/pause and single-step buttons, replay first-pass editor entity-list scrolling and selection, execute Luau-backed frame simulation, and evaluate first-pass field/editor assertions.
...
- [ ] Port remaining editor-shell input routing for Odin test replay, including system/inspector scrolling, inspector selection, splitter dragging, gizmo capture, and text editing.
