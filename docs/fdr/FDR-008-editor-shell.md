...
- Header bands, inspector surfaces, viewport seams, and selection use the shared ECS box border fields; pooled browser rows use hidden subtrees rather than leaving the ECS lifecycle. The default desktop density uses 13-pixel text, 32-pixel scene rows, 28-pixel inspector rows, and a 420-pixel inspector pane so labels and three-axis controls remain comfortable without becoming oversized.
