...
- While the shell is visible, every non-editor camera entity with a valid world Transform is represented in the scene viewport by a compact wireframe camera body, lens, and bounded projection frustum derived from its FOV, clip limits, and current viewport aspect. The visualization follows parented world transforms, highlights the selected camera, and excludes the separate editor-owned fly camera. Clicking a visible camera-body or frustum stroke selects its owning entity before ordinary rendered-triangle picking runs.
