...
- The top bar contains only the Scrapbot brand and Play, Pause, Stop, Step, and Save controls. Play captures the current authoring world and then runs project systems with normal frame deltas; Pause freezes project systems and world time while editor rendering and tools remain responsive; Stop restores the captured in-memory authoring world while retaining loaded Luau and Odin systems, then remains stopped; Step captures authoring state when necessary, advances one fixed 1/60-second project update, and remains paused; Save explicitly persists supported authored values while stopped.
...
- The inspector shows the selected entity's editable name, stable UUID, provenance, attached components, field names, and current values. Components are vertically stacked collapsible titled panels, and each panel renders its fields in an edge-to-edge, resizable two-column property table with a one-third label column and two-thirds value column inside an independently scrollable sidebar. Individual label and value cells provide the horizontal inset, keeping the table geometry aligned with the panel while its contents retain comfortable spacing. Values use reusable input controls: transform, camera, light, and custom Vec3 fields are live-editable, while unsupported field types remain selectable and read-only. Vec3 value cells compose three equal-width X, Y, and Z inputs with a fill HStack; scalar value cells contain one full-width input. A stopped-mode Add Component button opens a floating, independently scrollable menu composed from the same public layout, scroll-area, VStack, text, and button components. The menu reads the live component registry, nests dotted engine and library names by token, groups single-token project components under Project, marks membership with atlas-safe `[ ]` and `[x]` labels, and makes every supported membership change undoable.
...
- Completed inspector typing, stepping, scrubbing, boolean changes, transform-gizmo drags, entity operations, and component membership changes enter one bounded transaction history. `Ctrl/Cmd+Z` undoes, `Ctrl/Cmd+Shift+Z` redoes, and `Ctrl/Cmd+S` saves scene-authoring changes while the editor is open. Each complete gesture or structural action is one history step, Escape and invalid values do not create history, and editor shortcuts do not consume project input while chrome is closed or a project-owned input has focus.
...
**Tradeoff:** The single-world model restores authoring state only when playback stops rather than keeping it concurrently inspectable. A structurally rewritten entity block uses canonical TOML formatting, and runtime-generated resources are not automatically promoted into project resource declarations.
