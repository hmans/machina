...
- While the shell is visible, every non-editor camera entity with a valid world Transform is represented in the scene viewport by a world-scaled wireframe camera body and lens. The selected camera additionally shows a bounded projection-frustum preview derived from its FOV, near clip plane, current viewport aspect, and resolved world transform. The preview extends five world units or stops at a shorter far clip plane, preventing ordinary long camera ranges from dominating a zoomed-out scene view. The visualization naturally changes apparent size with viewpoint distance, highlights the selected camera, and excludes the separate editor-owned fly camera. Clicking a visible camera-body or selected frustum stroke selects its owning entity before ordinary rendered-triangle picking runs.
...
- The inspector shows the selected entity's editable name, stable UUID, provenance, attached components, field names, and current values. Components are vertically stacked collapsible titled panels, and each panel renders its fields in an edge-to-edge, resizable two-column property table with a one-third label column and two-thirds value column inside an independently scrollable sidebar. Individual label and value cells provide the horizontal inset, keeping the table geometry aligned with the panel while its contents retain comfortable spacing. Registry definitions may mark bookkeeping-heavy components as advanced; those components remain visible and fully inspectable but start collapsed whenever a pooled panel is rebound to them, while a user's expansion persists for the current binding. Registry fields with truthful Bool, String, Number, Vec2, Vec3, Vec4, or Color schemas are generically editable through reusable checkbox and input controls; vectors compose two to four equal-width axis inputs with a fill HStack. UUID-backed strings and text-alignment values use validated text until specialized entity and enum pickers exist. Built-in fields and custom numeric metadata provide immediate preview, constraints, and opt-in scrubbing. Engine-managed derived values and opaque render handles remain selectable and read-only. An Add Component button opens a floating, independently scrollable menu composed from the same public layout, scroll-area, VStack, text, and button components. The menu reads the live component registry, reuses cached registry name tokens, rebuilds its sorted definition cache only when the registry revision changes, nests dotted engine and library names by token, groups single-token project components under Project, and lists only components absent from the selected entity. Every removable component panel composes an ordinary direct-child icon button into its title band; activating that reusable button removes the component without toggling collapse. Stopped scene-entity membership changes are exact per-component undoable authoring transactions; running and paused changes apply immediately to scene or runtime entities as disposable playback state. Engine-defined scene components remain mutable because the entity owns their membership; engine-managed derived components remain inspector-visible and have no removal action.
...
- Entity membership, names, and resource-browser values refresh from the running world every 200 ms. The inspector refreshes immediately on selection and thereafter only when the selected entity component revision, selected resource version, or authoring mode changes. A focused input is not overwritten by a snapshot, and hover, scrolling, picking, gizmo input, and text editing remain frame-rate responsive.
...
**Decision:** Build component panels from the live registry and resolve each declared field against a typed entity snapshot. Use reusable checkboxes and inputs for every truthfully registered Bool, String, Number, Vec2, Vec3, Vec4, and Color field, then apply one validated snapshot when the edit commits. Preserve optimized direct adapters where useful, and keep opaque or engine-derived values read-only. Custom scalar/vector storage shares the same generic binding path and registry-declared editor metadata across project Luau, native Odin, scene TOML, and the editor.
...
**Tradeoff:** Generic reflected edits preview on commit rather than on every keystroke. UUID and alignment values are validated text controls until specialized pickers exist; Color has semantic RGBA storage and bounded channel inputs but not yet a public color-picker control. Resource handles, arrays, nested values, and components without truthful public field schemas need dedicated contracts.
...
**Decision:** Refresh scene and resource browsers at 5 Hz, but rebuild the formatted component inspector only when its selected entity component revision, selected resource version, selection, or authoring mode changes. Opening the editor or changing selection refreshes immediately. During a numeric scrub, mutate the bound ECS/resource value and input text directly and defer browser, profiler, and inspector snapshots until the gesture ends.
**Why:** Runtime entity churn requires periodic browser discovery, but it must not repeatedly reformat an unrelated stable inspector. Pointer feedback and manipulation remain frame-responsive.
**Tradeoff:** Passive browser changes can take up to 200 ms to appear. Inspector freshness depends on every supported component mutation incrementing the entity component revision, and profiler values intentionally remain frozen during a continuous scrub before refreshing on release.
...
**Decision:** Accumulate both tick-to-tick frame intervals and active CPU frame durations in fixed storage over independent rolling 50-frame windows, publish a revisioned diagnostics snapshot every five frames, and copy the renderer's latest GPU timestamp, draw-batch, instance, and visibility counters into that snapshot. Use guaranteed FIFO presentation for window pacing without a second fixed sleep. Acquire the presentation surface before active-frame timing so its expected vsync wait affects observed FPS but not the `FRAME` duration or Systems panel. Maintain scene, runtime, and editor entity counts at spawn/despawn boundaries so diagnostics never scan world capacity. Render the initial FPS, active CPU frame, GPU frame, project-entity, draw-batch, and occlusion-cull values through an editor-origin collapsible panel composed only from the public panel, table, VStack, layout, and text components.
