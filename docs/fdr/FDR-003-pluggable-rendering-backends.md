...
**Decision:** Render the world into a jittered floating-point target, reconstruct view-space geometry from the existing depth prepass, evaluate ambient occlusion at half resolution, blur it with separable depth-aware bilateral passes, fold it into the current HDR signal, camera-reproject that result into retained color/depth history with depth rejection and current-neighborhood clamping, build five successively smaller bloom levels from the temporal result, composite them with the HDR scene, tone map once, and draw UI afterward. Culling uses the unjittered camera, and resize, world replacement, depth replacement, or a camera cut rejects history.
**Why:** Architectural contacts and crevices need indirect-light grounding, bloom requires values above display white, broad halos need multiple spatial scales, subpixel geometry and texture detail need temporal supersampling, and text must remain crisp. Reusing depth avoids another geometry pass and supports camera reprojection without a separate velocity target. See ADR-029.
**Tradeoff:** Screen-space ambient occlusion cannot see off-screen occluders and currently attenuates the composited world rather than only indirect lighting. Camera-only temporal reprojection lacks exact motion for animated objects; previous-depth rejection and neighborhood clamping bound the resulting history error until per-object motion vectors exist. Ambient-occlusion, temporal, and bloom quality/weights remain fixed engine defaults. The compute paths require storage-texture support, full-resolution color/depth history consumes additional GPU memory, and the final composite remains a fullscreen render pass.
