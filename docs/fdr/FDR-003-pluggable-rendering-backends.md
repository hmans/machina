...
- Base-color and emissive images use sRGB sampling while metallic-roughness, normal, occlusion, and imported environment products remain linear. Diffuse irradiance and roughness-prefiltered specular reflection join direct GGX lighting and emission in a floating-point HDR target. `scrapbot.world_environment` selects lighting and an independent imported or procedural visible sky. The procedural atmosphere exposes sky/ground color, turbidity, thickness, horizon, and an HDR sun direction, color, intensity, disc, and glow. Sun elevation planet-occludes the disc and drives a day/twilight/night transition; the world shader analytically evaluates the same sky, ground, haze, and sun as roughness-aware diffuse and specular environment radiance through its ordinary split-sum GGX path. Above the horizon, the sun is also the first derived directional render light and therefore drives direct GGX lighting and the primary shadow map; explicit ECS lights remain additive. Half-resolution GTAO integrates horizon visibility from scene depth and mapped surface normals, filters only across compatible depth/normal neighborhoods, and attenuates the separately retained indirect-diffuse contribution. Direct lights, specular lighting, emission, and reflections are unaffected. World-environment exposure multiplied by active-camera exposure then reaches an eight-sample jittered, camera-reprojected temporal resolve with previous-depth rejection and neighborhood clamping. A five-level bloom chain and one tone-map pass produce the sRGB presentation target.
...
**Decision:** Render the world into floating-point color plus compact view-normal/roughness/metallic surface data and a separate indirect-diffuse contribution. Let the active camera opt into temporal antialiasing, lightweight current-frame fast antialiasing, half-resolution GTAO, material-aware screen-space reflections, and five-level bloom. GTAO searches both horizons across rotated view-space slices, analytically integrates their visibility, and crosses separable blur plus bilateral upsampling only through compatible depth and surface normals. It attenuates indirect diffuse rather than the complete HDR image. SSR reconstructs visible surfaces from depth and performs a bounded current-frame view-space ray march, fading uncertain, distant, rough, and screen-edge hits. Enabled TAA jitters the projection and camera-reprojects the AO/SSR-composed result into retained color/depth history with depth rejection and current-neighborhood clamping; it takes precedence over fast AA. Culling uses the unjittered camera, and resize, world replacement, depth replacement, camera cuts, or TAA mode changes reject history. The editor fly view inherits the active project camera's render switches. Disabled AO/SSR/bloom skip their compute passes, while disabled TAA skips jitter and history copies. Tone map once and draw UI afterward.
**Why:** Architectural contacts and crevices need indirect-light grounding, smooth materials need local reflections, bloom requires values above display white, broad halos need multiple spatial scales, subpixel geometry and texture detail need temporal supersampling, and text must remain crisp. Reusing depth supports AO, reflection ray intersection, and camera reprojection without another geometry pass or velocity target. See ADR-029.
**Tradeoff:** Screen-space effects cannot see off-screen or occluded sources. SSR currently uses a fixed bounded linear march and fades rough reflections instead of tracing Hi-Z or filtering them. Camera-only temporal reprojection lacks exact motion for animated objects; previous-depth rejection and neighborhood clamping bound the resulting history error until per-object motion vectors exist. Camera fields expose coarse switches, while AO/SSR/temporal/fast-AA/bloom quality and weights remain fixed engine defaults. The compute paths require storage-texture support; full-resolution surface, indirect-diffuse, reflection, color, and depth history consume additional GPU memory; and the final composite remains a fullscreen render pass.
...
**Decision:** Keep environment lighting and visible backgrounds independent. Preserve every imported Environment's source-resolution linear panorama for an optional infinite camera-oriented background, while using importer-built irradiance and prefiltered specular cubes for lighting. One authored `scrapbot.world_environment` component selects both sources plus independent intensity, rotation, exposure compensation, and blur. An enabled background without a resource renders a procedural atmospheric sky with a distinct spherical ground hemisphere, subtly curved aerial-perspective horizon, and an independently art-directed HDR sun. When no imported lighting Environment is selected, the world shader analytically evaluates that atmosphere for diffuse and roughness-aware specular environment radiance instead of using a material-independent ambient-specular constant. The component exposes bounded sky tint, ground color, turbidity, thickness, horizon softness, sun direction, color, intensity, size, and glow; these values share the retained environment revision and uniform update instead of creating per-frame work or generated cube resources. World-environment and active-camera exposure apply to the complete HDR world.
