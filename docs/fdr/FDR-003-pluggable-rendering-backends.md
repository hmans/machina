...
One optional authored `scrapbot.volumetric_fog` component supplies a global exponential height medium. The temporal resolve integrates 16 fixed midpoint samples up to scene depth or its distance bound. Ambient scattering is unshadowed; anisotropic primary-directional scattering uses a 2×2 UV-space filter and the same cascade transition bands as opaque geometry. Projects may independently opt clustered point lights into the medium. Absence or zero density takes a no-op branch and allocates no fog target.
...
TAA jitters the projection with an eight-sample sequence bounded to a quarter pixel and camera-reprojects the composed result into retained color/depth history. Stable views favor retained history to suppress visible jitter breathing. Resize, world replacement, depth replacement, camera cuts, and TAA mode changes reject history. Culling stays unjittered.
