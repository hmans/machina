...
**Decision:** Render the world into a floating-point target, reconstruct view-space geometry from the existing depth prepass, evaluate ambient occlusion at half resolution, blur it with separable depth-aware bilateral passes, apply contrast-adaptive FXAA to the world color, build five successively smaller bloom levels, composite them with the HDR scene, tone map once, and draw UI afterward.
**Why:** Architectural contacts and crevices need indirect-light grounding, bloom requires values above display white, broad halos need multiple spatial scales, geometry needs a low-cost antialiasing baseline, and text must remain crisp. Reusing depth avoids another geometry pass. See ADR-029.
**Tradeoff:** Screen-space ambient occlusion cannot see off-screen occluders and currently attenuates the composited world rather than only indirect lighting. FXAA is a spatial approximation rather than true multisampling or temporal reconstruction, while ambient-occlusion and bloom quality/weights remain fixed engine defaults. The compute paths require storage-texture support, and the final composite remains a fullscreen render pass.
