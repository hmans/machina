...
- Build validates the copied packaged project before reporting success. The current Zig path loads and registers packaged native artifacts; during the Odin migration, the Odin path loads preserved, newly built, or development-time Odin artifacts for registration, typed field callbacks, and deferred structural callbacks while source live reload remains pending.
