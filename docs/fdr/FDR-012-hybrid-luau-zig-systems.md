...
- During the Odin migration, project-local Odin native declarations can be declared with `native = "native/game.odin"` for static component/system validation in the Odin `check` path. Buildable Odin native modules import the generated facade with `import scrapbot "scrapbot:scrapbot_native"` and can export `scrapbot_register` with the C ABI for development-time and packaged artifact loading.
