...
- WGPU paints retained UI after world geometry, including in headless framegrabs. Project UI, editor chrome, and dynamic editor-world camera/gizmo primitives use independent revision-driven command, CPU-vertex, and GPU-buffer streams. Changing one stream does not hash, regenerate, or upload the others.
