...
- WGPU paints retained UI after world geometry, including in headless framegrabs. Dynamic editor-world camera and gizmo primitives use a separate small overlay command and vertex stream, so moving a viewpoint or selected transform does not invalidate retained project or editor chrome.
...
**Tradeoff:** The implementation has fixed node/paint limits. Stable project and editor domains use monotonic paint revisions plus compact focus/font state; unchanged domains retain their paint-command ranges and skip both traversal and glyph emission. WGPU separately signatures the retained command stream and viewport so unchanged frames neither rebuild nor upload UI geometry. Dynamic editor-world overlays are rebuilt into a bounded independent stream. Every visual, structural, layout, scrolling, or interaction mutation must increment the correct retained-domain revision through the typed ownership path so both cache layers remain correct. The same reconciler maintains distinct project and transient editor UI coordinate and interaction domains. See ADR-024.
