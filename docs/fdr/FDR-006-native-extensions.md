...
Native extensions let project code add compiled engine/library behavior incrementally. The first slice loads dynamic libraries from a project directory and lets them register component schemas into the same registry used by scene validation, Luau queries, and generated Luau types.
...
### 1. Load project-local build outputs
...
**Tradeoff:** Scrapbot does not yet build project extensions automatically during `scrapbot run`; users or tasks must place platform libraries there.

### 2. Start with schema registration only
...
### 3. Reuse library component ownership
...
- How should project build tasks compile and stage native extensions cross-platform?
