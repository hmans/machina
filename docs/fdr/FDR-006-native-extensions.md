...
**Tradeoff:** Native UI writes replace a complete typed payload after a read/modify/write cycle, strings have fixed maximum lengths, and a caller-owned spawn request contains at most the ABI's fixed component capacity. Once accepted, the engine queues only component payloads and schema fields actually present.
