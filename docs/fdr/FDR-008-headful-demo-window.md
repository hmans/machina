...
**Odin migration note:** The Odin `run` command can execute bounded hidden frame loops and, with `--backend wgpu`, write a final offscreen WebGPU frame artifact through the Odin `wgpu-native` path. The Odin SDL boundary can initialize video, create a hidden high-DPI window, and extract platform-native `wgpu-native` surface descriptors for Metal, Wayland/X11, or Win32. Odin still rejects visible WebGPU presentation and WebGPU editor chrome until the surface presentation loop is ported.
