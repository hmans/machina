...
**Odin migration note:** During the staged Odin rewrite, the Odin `test` command can discover test projects, validate manifests, replay script-visible input resources, route retained scene UI command/scroll input, consume editor-chrome pointer input before scripts observe it, replay editor play/pause and single-step buttons, replay first-pass editor entity-list/system-list/inspector scrolling, route inspector component scrolling through a retained scroll-view/vgroup model, entity selection, selected-header full-id copy, entity spawn/despawn controls, visible entity spawn/despawn button clicks, selected-entity component add/remove controls, visible selected-entity component add/remove button clicks, inspector field selection, splitter dragging, scalar and vec3-lane inspector text editing, text caret navigation and selection, inspector clipboard copy/paste, boolean toggles, known engine string selectors, inspector field undo/redo, invalid inspector edit diagnostics, and translate gizmo dragging, run Luau-backed frame simulation, and evaluate first-pass final ECS field plus editor-state assertions. Rich editor-shell interactions such as native-backed fixtures still remain in the Zig implementation until those Odin subsystems land. The Odin `render`, `render-test`, `render-bench`, and bounded hidden `run --backend wgpu` paths can now write first-pass image artifacts, metadata sidecars, selected-inspector component-card pixels for `--editor --select`, first-pass selected-inspector visual scroll pixels through `--inspector-scroll-y`, first-pass typed inspector control pixels for boolean, scalar, string, and vec3 values, first-pass WebGPU editor chrome overlay vertices from ECS render data, selected-inspector component-card plus typed-control overlay vertices for selected WebGPU editor renders, and first-pass in-memory software render benchmark timings with render extraction stats. The Odin `visual-test` command validates command shape, project loading, selected entity ids, expected/actual path safety, bounded Luau frame simulation, golden update, actual artifact output, image comparison, tolerance checks, metadata sidecars, render extraction stats, and first-pass inspector widget pixels on the software offscreen renderer path. Odin visible software `run` now creates an SDL window, pumps quit/close events, drives the shared live-project frame tick for bounded and unbounded runs, presents software-rendered scene plus first-pass editor chrome pixels through an SDL texture, and routes first-pass editor pointer/keyboard/text input through the shared runtime input model. Visible WebGPU `run` also presents scene-derived frames, first-pass editor chrome overlays, selected-inspector overlay vertices, and first-pass editor input routing through a persistent Odin `wgpu-native` surface context for bounded and unbounded runs, while the full editor window loop remains pending.
...
- `scrapbot render-bench [path] [--editor] [--frames N] [--warmup N] [--dt seconds] [--format text|json]` renders the default scene offscreen for warmup and measured frames, steps the live project between frames, and reports total and per-frame in-memory software render timing plus render extraction stats.
- `scrapbot render-bench [path] --editor --format=json` reports the measured frame count, warmup count, timestep, render target size, pixel scale, backend label, total nanoseconds, nanoseconds per frame, and render extraction stats so agents can verify first-pass render-performance changes before relying on manual headful testing. Per-render-system profiler samples remain migration-era Zig behavior until the Odin renderer profiler lands.
...
- `SCRAPBOT_LEAK_CHECK=1` enables an internal engine heap leak guard for bounded migration-era Zig CLI validation commands. It checks Scrapbot-owned Zig allocations and fails the command when command-scoped allocations leak.
