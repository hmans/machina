...
register_scrapbot_api :: proc(L: Lua_State) {
	...
	); lua_setfield(L, -2, "directional_light")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.point_light",
	); lua_setfield(L, -2, "point_light")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.shadow_caster",
	); lua_setfield(L, -2, "shadow_caster")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.shadow_receiver",
	); lua_setfield(L, -2, "shadow_receiver")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.ui_layout",
	...
	push_registered_component_handle_by_name(
		L,
		"scrapbot.ui_vstack",
	); lua_setfield(L, -2, "ui_vstack")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.ui_scroll_area",
	); lua_setfield(L, -2, "ui_scroll_area")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.ui_panel",
	...
	push_registered_component_handle_by_name(
		L,
		"scrapbot.ui_button",
	); lua_setfield(L, -2, "ui_button")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.ui_input",
	); lua_setfield(L, -2, "ui_input")
	push_registered_component_handle_by_name(
		L,
		"scrapbot.ui_checkbox",
	); lua_setfield(L, -2, "ui_checkbox")
	...
	lua_pushcclosurek(
		...
		"scrapbot.geometry.create",
		0,
		nil,
	); lua_setfield(L, -2, "create")
	lua_pushcclosurek(
		L,
		scrapbot_geometry_cube,
	...
	lua_pushcclosurek(
		L,
		scrapbot_geometry_plane,
		"scrapbot.geometry.plane",
		0,
		nil,
	...
	lua_pushcclosurek(
		...
		"scrapbot.geometry.icosphere",
		0,
		nil,
	); lua_setfield(L, -2, "icosphere")
	lua_pushcclosurek(
		L,
		scrapbot_geometry_sphere,
		"scrapbot.geometry.sphere",
		0,
		nil,
	); lua_setfield(L, -2, "sphere")
	lua_pushcclosurek(
		L,
		scrapbot_geometry_pyramid,
		"scrapbot.geometry.pyramid",
		0,
		nil,
	); lua_setfield(L, -2, "pyramid")
	lua_pushcclosurek(
		L,
		scrapbot_geometry_cylinder,
		"scrapbot.geometry.cylinder",
		0,
		nil,
	...
	lua_pushcclosurek(
		L,
		scrapbot_material_unlit,
		"scrapbot.material.lit",
		0,
		nil,
...
scrapbot_geometry_cube :: proc "c" (L: Lua_State) -> c.int {
	...
	if runtime == nil ||
	   ...
	   !ok { return luau_push_error(L, "geometry.cube expects a resource name") }
	size := f32(
		1,
	); if lua_gettop(L) >= 2 { is_number: c.int; size = f32(lua_tonumberx(L, 2, &is_number)); if is_number == 0 { return luau_push_error(L, "cube size must be a number") } }
...
scrapbot_geometry_create :: proc "c" (L: Lua_State) -> c.int {
	...
	if runtime == nil ||
	   ...
	   !name_ok ||
	   lua_type(L, 2) !=
		   LUA_TTABLE { return luau_push_error(L, "geometry.create expects a name and descriptor") }
	vertices: [dynamic]resources.Vertex; indices: [dynamic]u32
	defer delete(vertices); defer delete(indices)
	lua_getfield(L, 2, "vertices")
	if lua_type(L, -1) !=
	   LUA_TTABLE { lua_settop(L, -2); return luau_push_error(L, "geometry vertices must be an array") }
	for i := 1; i <= 65536; i += 1 {
		lua_rawgeti(L, -1, c.int(i)); if lua_type(L, -1) == LUA_TNIL { lua_settop(L, -2); break }
		if lua_type(L, -1) !=
		   LUA_TTABLE { return luau_push_error(L, "geometry vertices must be tables") }
		lua_getfield(
			L,
			-1,
			"position",
		); position, position_ok := vec3_argument(L, -1); lua_settop(L, -2)
	...
	lua_getfield(
		...
		2,
		"indices",
	); if lua_type(L, -1) != LUA_TTABLE { lua_settop(L, -2); return luau_push_error(L, "geometry indices must be an array") }
	for i := 1; i <= 196608; i += 1 {
		lua_rawgeti(L, -1, c.int(i)); if lua_type(L, -1) == LUA_TNIL { lua_settop(L, -2); break }
		is_number: c.int; value := lua_tointegerx(L, -1, &is_number); lua_settop(L, -2)
		if is_number == 0 ||
		   value <
			   0 { return luau_push_error(L, "geometry indices must be non-negative integers") }; append(&indices, u32(value))
	}
	lua_settop(L, -2)
...
scrapbot_geometry_icosphere :: proc "c" (L: Lua_State) -> c.int {
	...
	name, ok := luau_required_string(
		L,
		1,
	); if runtime == nil || runtime.resource_registry == nil || !ok { return luau_push_error(L, "geometry.icosphere expects a resource name") }
	radius, radius_ok := optional_f32(
		L,
		2,
		0.5,
	); subdivisions, subdivisions_ok := optional_int(L, 3, 2)
	if !radius_ok ||
	   !subdivisions_ok { return luau_push_error(L, "icosphere radius and subdivisions must be numbers") }
	desc, err := resources.icosphere(
		radius,
		subdivisions,
	); return register_generated_luau_geometry(L, runtime, name, desc, err)
}
...
scrapbot_geometry_sphere :: proc "c" (L: Lua_State) -> c.int {
	...
	name, ok := luau_required_string(
		L,
		1,
	); if runtime == nil || runtime.resource_registry == nil || !ok { return luau_push_error(L, "geometry.sphere expects a resource name") }
	radius, radius_ok := optional_f32(
		L,
		2,
		0.5,
	); segments, segments_ok := optional_int(L, 3, 24); rings, rings_ok := optional_int(L, 4, 16)
	if !radius_ok ||
	   !segments_ok ||
	   !rings_ok { return luau_push_error(L, "sphere radius, segments, and rings must be numbers") }
	desc, err := resources.sphere(
		radius,
		segments,
		rings,
	); return register_generated_luau_geometry(L, runtime, name, desc, err)
...
scrapbot_geometry_pyramid :: proc "c" (L: Lua_State) -> c.int {
	...
	name, ok := luau_required_string(
		L,
		1,
	); if runtime == nil || runtime.resource_registry == nil || !ok { return luau_push_error(L, "geometry.pyramid expects a resource name") }
	width, width_ok := optional_f32(
		L,
		2,
		1,
	); height, height_ok := optional_f32(L, 3, 1); depth, depth_ok := optional_f32(L, 4, 1)
	if !width_ok ||
	   !height_ok ||
	   !depth_ok { return luau_push_error(L, "pyramid dimensions must be numbers") }
	desc, err := resources.pyramid(
		width,
		height,
		depth,
	); return register_generated_luau_geometry(L, runtime, name, desc, err)
...
scrapbot_geometry_cylinder :: proc "c" (L: Lua_State) -> c.int {
	...
	name, ok := luau_required_string(
		L,
		1,
	); if runtime == nil || runtime.resource_registry == nil || !ok { return luau_push_error(L, "geometry.cylinder expects a resource name") }
	radius, radius_ok := optional_f32(
		L,
		2,
		0.5,
	); height, height_ok := optional_f32(L, 3, 1); segments, segments_ok := optional_int(L, 4, 24)
	if !radius_ok ||
	   !height_ok ||
	   !segments_ok { return luau_push_error(L, "cylinder radius, height, and segments must be numbers") }
	desc, err := resources.cylinder(
		radius,
		height,
		segments,
	); return register_generated_luau_geometry(L, runtime, name, desc, err)
...
register_generated_luau_geometry :: proc(
	L: Lua_State,
	runtime: ^Runtime,
	name: string,
	desc: resources.Geometry_Desc,
	generation_err: string,
) -> c.int {
...
optional_f32 :: proc "c" (L: Lua_State, index: c.int, default: f32) -> (f32, bool) {
	if lua_gettop(L) <
	   index { return default, true }; ok: c.int; value := f32(lua_tonumberx(L, index, &ok)); return value, ok != 0
}

optional_int :: proc "c" (L: Lua_State, index: c.int, default: int) -> (int, bool) {
	if lua_gettop(L) <
	   index { return default, true }; ok: c.int; value := int(lua_tointegerx(L, index, &ok)); return value, ok != 0
}
...
scrapbot_material_textured :: proc "c" (L: Lua_State) -> c.int {
	...
	if runtime == nil ||
	   ...
	   !name_ok ||
	   !path_ok { return luau_push_error(L, "material.textured expects a resource name and project asset path") }
	values := [4]f32{1, 1, 1, 1}
	...
	handle, err := resources.register_textured_material(
		runtime.resource_registry,
		runtime.project_root,
		name,
		path,
